...
# umanag - Basic User Management tool for Unix
# Uses Zenity for a simple GUI interface.

# Check for root privileges
if [[ $EUID -ne 0 ]]; then
   zenity --error --text="This application must be run as root (use sudo)."
   exit 1
fi

main_menu() {
    while true; do
        CHOICE=$(zenity --list --title="umanag - User Management" \
